...
@section('title', 'Edit Bidan / Terapis')
@section('page-title', 'Edit Bidan / Terapis')
@section('breadcrumb', 'Edit Bidan / Terapis')
...
@section('content')
    ...
    <div class="card border-0 rounded-lg max-w-4xl">
        <div class="card-body p-6">
            <h6 class="font-bold text-lg mb-4">Form Edit Bidan / Terapis</h6>

            <form action="{{ route('admin.bidans.update', $bidan) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="credential" id="credential"
                                   value="{{ old('credential', $bidan->credential) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="specialization" id="specialization"
                                   value="{{ old('specialization', $bidan->specialization) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="str_number" id="str_number"
                                   value="{{ old('str_number', $bidan->str_number) }}"
                        ...
                        <div>
                            ...
                            <input type="number" name="experience_years" id="experience_years"
                                   value="{{ old('experience_years', $bidan->experience_years) }}"
                        ...
                        <div class="flex items-center gap-2 mt-6">
                            ...
                            <input type="checkbox" name="is_active" id="is_active" value="1"
                                   class="form-check-input" {{ old('is_active', $bidan->is_active) ? 'checked' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <div>
                        @if ($bidan->photo_url)
                            <div class="mb-3">
                                <img src="{{ $bidan->photo_url }}" alt="{{ $bidan->name }}" class="w-24 h-24 rounded-lg object-cover">
                                <p class="text-sm text-neutral-400 mt-1">Foto saat ini. Upload foto baru untuk mengganti.</p>
                            </div>
                        @endif
                        <label for="photo" class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
                            Upload Foto Baru <span class="text-neutral-400 font-normal">(maks 2MB, format: JPG, PNG, WebP)</span>
                        </label>
                ...
                <div class="mb-6">
                    ...
                    <div>
                        ...
                        <textarea name="bio" id="bio" rows="3"
                                  ...
                                  placeholder="Deskripsi singkat tentang bidan/terapis">{{ old('bio', $bidan->bio) }}</textarea>
                        @error('bio')
                            <p class="text-danger-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="mb-6">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="address" id="address" rows="2"
                                  ...
                                  placeholder="Alamat lengkap">{{ old('address', $bidan->address) }}</textarea>
                        @error('address')
                            <p class="text-danger-600 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <input type="text" name="latitude" id="latitude"
                                   value="{{ old('latitude', $bidan->latitude) }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="longitude" id="longitude"
                                   value="{{ old('longitude', $bidan->longitude) }}"
                        ...
                        @foreach ($days as $key => $label)
                            <div class="flex items-center gap-3 flex-wrap">
                                ...
                                <input type="time" name="schedule[{{ $key }}][start]"
                                       value="{{ old("schedule.{$key}.start", $bidan->schedule[$key]['start'] ?? '') }}"
                                       class="form-control rounded-lg text-sm w-32">
                                <span class="text-neutral-400">-</span>
                                <input type="time" name="schedule[{{ $key }}][end]"
                                       value="{{ old("schedule.{$key}.end", $bidan->schedule[$key]['end'] ?? '') }}"
                                       class="form-control rounded-lg text-sm w-32">
                            </div>
                        @endforeach
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        const initialLat = {{ old('latitude', $bidan->latitude) ?: 'null' }};
        const initialLng = {{ old('longitude', $bidan->longitude) ?: 'null' }};
